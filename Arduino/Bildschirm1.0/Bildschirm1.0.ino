...
void readSignal(void) {
  ...
  if(arduinoReady == false) {
    ...
    if(incommingDataArd.length() > 1) {
      Serial.println("arduino ready");
      connectionCounter++;
    }
    if(incommingDataRasp.length() > 1) {
      Serial.println("rasp ready");
      connectionCounter++;
    }
  ...
  */
  ...
  if (mode == 1) {
    ...
    if (endOfCommandPos > 0) {
      ...
      long analyseStartTime = millis();
      int i = 0;
      while (command.length() != 0) {
        Serial.print("remaining command: ");
        Serial.println(command);

        //skip if command cant be reduced cause wrong read;
        if(analyseStartTime + 50 < millis()) {
          Serial.print("Skipped Command -------------------------------------------------");
          command = "";
          break;
        }
